...
const CONFIG_DEFAULT = {
  ...
  descripcion: 'Toca la imagen para hacer tu pedido',
  empresa: 'DISTRILECO',
...
function enviarPedido(){
  ...
  const pedido = {
    ...
    nombre, direccion, celular, pago, obs,
    gps: gpsActual,
    estado: 'pendiente'
  };
    ...
    return `
      <div class="pedido-card">
        ...
        <div class="pedido-acciones">
          ...
          ${mapaLink ? `<button class="btn-mapa" onclick="window.open('${mapaLink}','_blank')">Mapa</button>` : ''}
...
function guardarEdicion(){
  ...
  if(idx === -1) return;
  pedidos[idx].nombre = document.getElementById('e_nombre').value.trim();
  pedidos[idx].direccion = document.getElementById('e_direccion').value.trim();
  pedidos[idx].celular = document.getElementById('e_celular').value.trim();
